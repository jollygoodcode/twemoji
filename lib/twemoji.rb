...
module Twemoji
  ...
  private
    ...
    def self.img_tag(name)
      ...
      if options[:img_attr]
        "<img class='#{options[:class_name]}' draggable='false' title='#{title}' alt='#{alt}' #{options[:img_attr]} src='#{emoji_url(name)}'>"
      else
        "<img class='#{options[:class_name]}' draggable='false' title='#{title}' alt='#{alt}' src='#{emoji_url(name)}'>"
      end
    end

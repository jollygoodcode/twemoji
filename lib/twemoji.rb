...
module Twemoji
  ...
  private
    ...
    def self.filter_emojis_unicode(content)
      content.gsub(emoji_pattern_unicode) { |match| img_tag_unicode(match) }
    end
    ...
    def self.img_tag_unicode(name)
      img_attrs_hash = default_attrs_unicode(name).merge! customized_attrs(name)
    ...
    def self.default_attrs_unicode(name)
      {
        draggable: "false".freeze,
        title:     find_by_unicode(name),
        alt:       name,
        src:       emoji_url(name, :unicode_to_code)
      }
    end
